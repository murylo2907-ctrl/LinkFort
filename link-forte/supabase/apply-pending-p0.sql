-- =============================================================================
-- P0 pendente — cole TUDO no SQL Editor do Supabase e clique em Run
-- Pré-requisito: 001_partner_portal.sql já aplicada; usuário admin criado em Auth
-- =============================================================================
--
-- PASSO 0 (opcional): veja os usuários em Authentication → Users, ou rode:
--   SELECT id, email, created_at FROM auth.users ORDER BY created_at;
--
-- PASSO 1: na PARTE 2 abaixo, troque 'SEU-EMAIL-ADMIN@exemplo.com' pelo e-mail
--          exato do usuário admin.
--
-- =============================================================================

-- =============================================================================
-- PARTE 1 — Migration 002 (partner_invites)
-- =============================================================================
...
create unique index if not exists partner_invites_open_email_idx
  on public.partner_invites (email)
  where claimed_at is null;

alter table public.partner_invites enable row level security;

drop policy if exists "Admin gerencia convites" on public.partner_invites;
...
-- =============================================================================
-- PARTE 2 — Vincular usuário admin em partner_roles
-- Troque o e-mail abaixo pelo do seu admin (Authentication → Users)
-- =============================================================================

DO $$
DECLARE
  v_admin_email text := 'SEU-EMAIL-ADMIN@exemplo.com';  -- <<< ALTERE AQUI
  v_admin_id uuid;
BEGIN
  ...
  IF v_admin_id IS NULL THEN
    RAISE EXCEPTION 'Usuário admin não encontrado para o e-mail: %', v_admin_email;
  END IF;
  ...
  RAISE NOTICE 'Admin vinculado: % (%)', v_admin_email, v_admin_id;
END $$;

-- =============================================================================
-- PARTE 3 — Verificação (deve mostrar 1 linha em partner_roles com role admin)
-- =============================================================================

SELECT EXISTS (
  SELECT 1 FROM information_schema.tables
  WHERE table_schema = 'public' AND table_name = 'partner_invites'
) AS partner_invites_ok;

SELECT user_id, role FROM public.partner_roles WHERE role = 'admin';
